...
def read_in_data_CG():
	fl = open("letterCG.data")
	data_points = []
	class_labels = []
	for line in fl:
		# attributes distinguishing C's and G's
		line = line.split()
		for i in range(len(line)):
			if line[i] == 'C':
				line[i] = -1
...
#takes in threshold and makes predictions for that stump, returns predictions
...
#takes in a weight vector D, our data, the labels, the error for this time and the class estimate
def build_decision_stump(D, data_points, class_labels, error, class_estimate):
	# Min error previously set to infinity
...
def calc_D(weak_stump, class_labels, class_estimate, D):
	...
	#find the exponent vector to multiply D by. This took ages to understand.
...
def train_decision_stumps(data_points, class_labels, num_iterations):
	...
	# make the weight value vector D
	D = []
	for i in range(num_items):
...
def main():
	...
	print
	print "Adaboost on C, G letter identification"
	data_points, class_labels = read_in_data_CG()
